...
from sqlalchemy import (
    ...
    Float, Boolean, ForeignKey, Table, LargeBinary, Index
)
...
class EpisodicStoreV2:
    ...
    def append_memory(
        ...
        try:
            if skip_duplicates:
                existing = session.query(MemoryLogV2).filter(
    ...
    def get_by_id(self, memory_id: int, user_id: Optional[str] = None) -> Optional[MemoryLogV2]:
    ...
    def toggle_bookmark(self, memory_id: int, user_id: Optional[str] = None) -> Optional[bool]:
    ...
    def add_tags(self, memory_id: int, tags: List[str], user_id: Optional[str] = None) -> bool:
    ...
    def get_stats(self, user_id: Optional[str] = None) -> dict:
        ...
        try:
            ...
            return {
                ...
                "storage_saved_percent": round((1 - total_compressed / total_original) * 100, 2) if total_original > 0 else 0,
                "average_compression_ratio": round(total_original / total_compressed, 2) if total_compressed > 0 else 1,
                "sources": sources,
            }
        finally:
            session.close()
    ...
    def get_search_history(self, user_id: Optional[str] = None, limit: int = 20) -> List[SearchHistory]:
